...
class LPD(nn.Module):
    ...
    def getLoss(self,src,src_embedding,tgt_embedding):
        ...
        # 获得索引阶梯数组
        idx_base = torch.arange(0, batch_size, device=torch.device('cuda')).view(-1, 1,1) * k  # (batch_size, 1, 1) [0 k ... k*(B-1)]
        # 以batch为单位，加到索引上
        idx = idx + idx_base  # (batch_size, k, nk)
        # 展成一维数组，方便后续索引
        idx = idx.view(-1)  # (batch_size * k * nk)
        # 改变x的shape，方便索引。被索引数组是所有batch的所有点的特征，索引数组idx为所有临近点对应的序号，从而索引出所有领域点的特征
        topFarTgt = tgt_embedding_k.transpose(2, 1).contiguous().view(batch_size * k, -1)[idx, :]  # (batch_size * k * nk,num_dims)
        # 统一数组形式
        src_embedding_shaped = src_embedding_k.transpose(2, 1).contiguous().view(batch_size, k, 1, num_dims).repeat((1, 1, nk, 1)).view(batch_size*k*nk,-1)
        tgt_embedding_shaped = tgt_embedding_k.transpose(2, 1).contiguous().view(batch_size, k, 1, num_dims).repeat((1, 1, nk, 1)).view(batch_size*k*nk,-1)
...
def test_one_epoch(args, net, test_loader):
    ...
    with torch.no_grad():
        for src, target, rotation_ab, translation_ab, rotation_ba, translation_ba, euler_ab, euler_ba, label in tqdm(test_loader):
            src = src.cuda()
            target = target.cuda()
            batch_size = src.size(0)
...
def train_one_epoch(args, net, train_loader, opt):
    ...
    for src, target, rotation_ab, translation_ab, rotation_ba, translation_ba, euler_ab, euler_ba, label in tqdm(train_loader):
        src = src.cuda()
        target = target.cuda()
        batch_size = src.size(0)
        opt.zero_grad()
